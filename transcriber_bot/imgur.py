...
IMGUR_URLS = ["https://imgur.com/", "https://i.imgur.com/"]
QUOTE_CHARS = "/:"
...
def is_url_imgur(imgur_url):
    ...
    """
    if imgur_url:
        imgur_url = quote(imgur_url, safe=QUOTE_CHARS)
    else:
        return False
    for valid_url in IMGUR_URLS:
        if imgur_url.startswith(valid_url):
            return True
    return False
...
def get_imgur_url(imgur_image):
    ...
    imgur_animated = imgur_image.get("animated")

    # checks if the imgur image object is not animated and has a "hash" and
    # "ext" within it
    if not imgur_animated and imgur_hash and imgur_ext:
        return "{url}{hash}{ext}".format(url=IMGUR_URLS[0],
                                         hash=imgur_hash,
                                         ext=imgur_ext)
